            ...
            while True:
                try:
                    ...
                    if count > retry:
                        ...
                        print("sid invalid, retry %s" % count)
                        self.login()
...
class JoyrunClient(object):
    ...
    def __init__(self,args):
        ...
        self.dis = float(args[3])
        self.pace = float(args[4])
        self.freq = int(args[5])
        self.type = args[6]
    ...
    def __reqeust(self, method, url, **kwargs):
        ...
        print("request.url = %s" % resp.url)
        print("response.json = %s" % pretty_json(respJson))
    ...
    def run(self):
        """ 项目主程序外部调用接口
        """
        distance = self.dis # 总距离 km
        pace = self.pace # 速度 min/km
        stride_frequncy = self.freq # 步频 step/min
        record_type = self.type # 跑步记录类型
        record_number = 1 # 跑步记录编号

        if record_type == "dongcao":
            record_instances = record.__Record_DongCao_Instance__
        elif record_type == "xicao":
            record_instances = record.__Record_XiCao_Instance__
        elif record_type == "random":
            record_instances = getattr(record, random.choice(["__Record_DongCao_Instance__","__Record_XiCao_Instance__"]))
        else:
            raise RecordTypeError("unsupport record type '%s' ! valid type = ['random','dongcao','xicao']" % record_type)
